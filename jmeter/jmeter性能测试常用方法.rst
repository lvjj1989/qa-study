...
从 Jenkins 中获取参数到 JMeter 脚本
----------------------------------------

在 Jenkins 中可以添加配置参数，这些参数会加入到临时的环境变量中，在 Jenkins 中通过 BeanShell 脚本便可以获取到 Jenkins 的参数。

在 Jenkins 中添加你需要透传给 JMeter 的参数
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
在 JMeter 中添加一个前置处理器 BeanShell PreProcessor，输入以下代码：
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
在 JMeter 脚本中通过 `${DataId}` 获取 Jenkins 中配置的 `data_id`。
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~


BeanShell 断言：在报告中展示异常的 response
------------------------------------------------------------------------

在性能测试中，如果只使用响应断言，那么在用例失败时，我们只知道响应里没有需要的断言数据，却不知道实际响应了什么。这时候就可以把响应断言修改成下面的 BeanShell 断言，这样失败时 FailureMessage 就会出现在报告中。
...
多个字符串断言：如 Str1 和 Str2 两者存在其一即为通过，代码如下：
...
JMeter 使用 BeanShell 预处理器从指定列表中获取随机值
----------------------------------------------------------------------
...
然后在脚本中使用 `${mynation}` 或者 `${anum}` 便可以获取对应参数。

.. warning:: 数量较多时推荐使用csv，当然也可以使用随机函数，但是在性能测试中不推荐使用随机函数，部分随机函数生成时的性能较差，无法提供足够的负载

仅一次控制器
-------------------------------------------------------------------------------
在编写性能测试时，有些场景我们只希望执行一次（如登录），这时候就可以使用【仅一次控制器】，将登录接口加入到仅一次控制器中。这样每个线程只会执行一次登录。
